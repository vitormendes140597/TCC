...
df6 <- SparkR::sql("Select
                      ...
                      weekday,
                      month,
                      mean(data) as mediaRaw,
                      mean(ppm) as mediaHoraria,
                      max(data) as maximaRaw,
                      max(ppm) as maximaPpm,
                      min(data) as minRaw,
                      min(ppm) as minimoPpm
                    From df
                    Group By date,time,city,weekday,month")

dfCopy <- df5
rv <- reactiveValues(df = df5)
server <- function(input, output) {
  ...
  output$table <- DT::renderDataTable({
    ...
    datatable(SparkR::collect(rv$df),class = "cell-border")
  })
  ...
  output$clean.choose_NumericVars <- renderUI({
    columns <- colnames(select_if(SparkR::collect(rv$df),is.numeric))
